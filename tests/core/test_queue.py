...
def test_redis_connection_initialization():
    with patch("app.core.queue.redis.from_url") as mock_from_url, \
...
def test_task_queue_with_redis_connection():
    ...
    with patch("app.core.queue.redis_conn", mock_redis), \
...
def test_redis_connection_error():
    with patch("app.core.queue.redis.from_url", side_effect=redis.ConnectionError("Connection refused")):
