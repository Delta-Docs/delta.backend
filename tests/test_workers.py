...
def test_start_worker_single():
    ...
    with patch("workers.Worker", return_value=mock_worker) as mock_worker_class, \
...
def test_main_single_worker():
    ...
    with patch("workers.settings", mock_settings), \
...
def test_worker_exception_during_work():
    ...
    with patch("workers.Worker", return_value=mock_worker), \
         patch("workers.task_queue", MagicMock()), \
         patch("workers.redis_conn", MagicMock()):
...
def test_redis_connection_shared():
    ...
    with patch("workers.Worker", side_effect=[mock_worker1, mock_worker2]) as mock_worker_class, \
         patch("workers.task_queue", MagicMock()), \
         patch("workers.redis_conn", mock_redis_conn):
